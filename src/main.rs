...
fn main() {
    ...
    let lambertian_green = materials::Lambertian { albedo: Vec3::new(0.8f64, 0.8f64, 0.3f64) };
    let lambertian_blue = materials::Lambertian { albedo: Vec3::new(0.1f64, 0.2f64, 0.5f64) };
    let metal_1 = materials::Metal {
        albedo: Vec3::new(0.8f64, 0.6f64, 0.2f64),
        fuzziness: 0.35f64,
    };
    let dielectric = materials::Dielectric { refraction_index: 1.5f64 };

    let mut world = RenderList::new();
    let small_sphere = shapes::Sphere {
        center: Vec3::new(0f64, 0f64, -1f64),
        radius: 0.5f64,
    };
    let right_sphere1 = shapes::Sphere {
        center: Vec3::new(-1f64, 0f64, -1f64),
        radius: 0.5f64,
    };
    let right_sphere2 = shapes::Sphere {
        center: Vec3::new(-1f64, 0f64, -1f64),
        radius: -0.45f64,
    };
    let left_sphere = shapes::Sphere {
        center: Vec3::new(1f64, 0f64, -1f64),
        radius: 0.5f64,
    };
    let big_sphere = shapes::Sphere {
        center: Vec3::new(0f64, -100.5f64, -1f64),
        radius: 100f64,
    };
    world.add_sphere(&small_sphere, &lambertian_blue);
    world.add_sphere(&big_sphere, &lambertian_green);
    ...
    // render
    {
        let render_settings = renderer::RenderSettings { num_samples_per_pixel: 100 };
